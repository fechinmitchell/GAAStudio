...
class Config:
    ...
    DEBUG = False
    TESTING = False
    JSON_DATASET_PATH = '/Users/fechinmitchell/Projects/GAAStudio/backend'
    IMAGE_DATASET_PATH = '/Users/fechinmitchell/Projects/GAAStudio/datasets'
...
class ProductionConfig(Config):
    ...
    JSON_DATASET_PATH = '/var/www/your_production_path/backend'
    IMAGE_DATASET_PATH = '/var/www/your_production_path/datasets'

app = Flask(__name__)
app.config.from_object(DevelopmentConfig() if os.getenv('FLASK_ENV') != 'production' else ProductionConfig())
CORS(app)

# Load the player stats JSON file
...
# Individual routes for each type of map or chart
@app.route('/pressuremaps/<team>')
def get_pressure_map(team):
    file_path = os.path.join(app.config['IMAGE_DATASET_PATH'], 'Pressure', f"{team}_Pressure_Map.png")
...
def get_tackle_map(team):
    file_path = os.path.join(app.config['IMAGE_DATASET_PATH'], 'Tackles', f"{team}_unsuccesful_tackles.png")
    if os.path.isfile(file_path):
        return send_from_directory(os.path.dirname(file_path), os.path.basename(file_path))
    else:
...
def get_blocked_map(team):
    file_path = os.path.join(app.config['IMAGE_DATASET_PATH'], 'Blocked', f"{team}_block_points.png")
    if os.path.isfile(file_path):
        return send_from_directory(os.path.dirname(file_path), os.path.basename(file_path))
    else:
...
def get_sdpm_chart(team):
    file_path = os.path.join(app.config['IMAGE_DATASET_PATH'], 'SDPM', f"{team}_SDPM_Chart.png")
    if os.path.isfile(file_path):
        return send_from_directory(os.path.dirname(file_path), os.path.basename(file_path))
    else:
        return jsonify({'error': 'SDPM map not found'}), 404

@app.route('/asdmap/<team>')
def get_asd_map(team):
    file_path = os.path.join(app.config['IMAGE_DATASET_PATH'], 'ASD', f"{team}_average_shot_positions.png")
    if os.path.isfile(file_path):
        return send_from_directory(os.path.dirname(file_path), os.path.basename(file_path))
    else:
...
def get_pp_map(team):
    file_path = os.path.join(app.config['IMAGE_DATASET_PATH'], 'PP', f"{team}_pressure_points.png")
    if os.path.isfile(file_path):
        return send_from_directory(os.path.dirname(file_path), os.path.basename(file_path))
    else:
...
def get_heatmap(team, category):
    file_path = os.path.join(app.config['IMAGE_DATASET_PATH'], category.capitalize(), f"{team}_{category.lower()}_heat_map.jpg")
    if os.path.isfile(file_path):
        return send_from_directory(os.path.dirname(file_path), os.path.basename(file_path))
    else:
        return jsonify({'error': f'{category.capitalize()} map not found'}), 404

# Load dataset for scoring efficiency calculations
player_stats_df = pd.read_json(os.path.join(app.config['JSON_DATASET_PATH'], 'DS_Player_Stats_Sorted_by_Difference_with_xPoints.json'))
