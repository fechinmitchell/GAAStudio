      ...
      onComplete: function () {
        ...
        this.data.datasets.forEach(function (dataset) {
          for (let i = 0; i < dataset.data.length; i++) {
            ...
            const scale_max = dataset._meta[Object.keys(dataset._meta)[0]].data[i]._yScale.maxHeight;
            const y_pos = model.y - 5;
            const x_pos = model.x;
            // Ensure the percentage is only shown if the bar is tall enough
            if ((scale_max - model.y) > 20) {
              ctx.fillStyle = '#444'; // Text color
              ctx.fillText(dataset.data[i].toFixed(2) + '%', x_pos, y_pos);
            }
          }
        });
      }
