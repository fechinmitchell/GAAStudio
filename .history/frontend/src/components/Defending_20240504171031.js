...
function Defending({ selectedTeam }) {
    ...
    const [blockedMapUrl, setblockedMapUrll] = useState('');
    const [tacklesMapUrlUrl, settacklesMapUrl] = useState('');
    ...
    useEffect(() => {
        if (selectedTeam) {
            const url = `/blockedmaps/${encodeURIComponent(selectedTeam)}`;
            setblockedMapUrll(url);
        }
    }, [selectedTeam]);
    ...
    useEffect(() => {
        if (selectedTeam) {
            const url = `/tacklesmaps/${encodeURIComponent(selectedTeam)}`;
            settacklesMapUrl(url);
        }
    }, [selectedTeam]);
        ...
        <div className="defensive-container" style={{ display: 'flex', justifyContent: 'space-between', padding: '20px' }}>
            ...
            <div className="graph-container" style={{ width: '30%' }}>
                {selectedTeam && tacklesMapUrl ? (
                    <Image src={tacklesMapUrl} alt={`${selectedTeam} Pressure Map`} fluid style={{ maxWidth: '100%', height: 'auto' }} />
                ) : (
