...
function Attacking({ heatMapUrl, setDataType, selectedTeam }) {
  ...
  // Fetching both scoring zone efficiency and player stats
  useEffect(() => {
    const fetchScoringZoneEfficiency = async () => {
      try {
        const response = await axios.get(`/scoring-zone-efficiency?team=${encodeURIComponent(selectedTeam)}`);
        if (response.data) {
          setScoringZoneEfficiency({
            ...scoringZoneEfficiency,
            datasets: [{
              ...scoringZoneEfficiency.datasets[0],
              data: [response.data.inside * 100, response.data.outside * 100]
            }]
          });
        }
      } catch (error) {
