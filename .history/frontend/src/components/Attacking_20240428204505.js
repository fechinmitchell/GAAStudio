...
function Attacking({ scoringZoneEfficiency, heatMapUrl, setDataType, selectedTeam }) {
    ...
    if (selectedTeam) {
      const fetchData = async () => {
        try {
          ...
          console.log('Data fetched for team:', selectedTeam, response.data);
          setPlayerStats(response.data);
        } catch (error) {
          console.error('Error fetching player stats for team:', selectedTeam, error);
        }
      };
      fetchData();
    }
  ...
  const sortedPlayerStats = React.useMemo(() => {
    if (!sortConfig.key) return playerStats;
  ...
  return (
    <div className="content-container" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px' }}>
      ...
      <Table striped bordered hover size="sm" className="player-stats-table-container">
        <thead>
          <tr>
