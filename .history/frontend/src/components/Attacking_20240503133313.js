...
useEffect(() => {
  if (selectedTeam) {
      const url = `/asdmap/${encodeURIComponent(selectedTeam)}`;
      setASDMapUrl(url);
  }
}, [selectedTeam]);
  ...
  return (
    <div className="content-container" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px' }}>
      ...
      <div className="attacking-container" style={{ display: 'flex', justifyContent: 'space-between', padding: '20px' }}>
            ...
            <div className="graph-container" style={{ width: '37.5%' }}>
                {selectedTeam && ASDMapUrl ? (
                    <Image src={ASDMapUrl} alt={`${selectedTeam} Pressure Map`} fluid style={{ maxWidth: '100%', height: 'auto' }} />
                ) : (
