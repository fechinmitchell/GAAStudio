...
function Attacking({ scoringZoneEfficiency, heatMapUrl, setDataType }) { // Ensure setDataType is received here
  ...
  return (
    <div className="content-container" style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', flexWrap: 'wrap' }}>
      <div className="chart-container" style={{ flex: 1, minWidth: '300px', margin: '10px' }}>
        <Bar data={chartData} options={chartOptions} />
      </div>
      <div className="heatmap-container" style={{ flex: 1, minWidth: '300px', margin: '10px', textAlign: 'center' }}>
        <Image src={heatMapUrl} alt="Heat Map" fluid />
        <div className="button-group" style={{ marginTop: '10px' }}>
          <Button variant="primary" onClick={() => setDataType('shots')}>Shots</Button>
          <Button variant="secondary" onClick={() => setDataType('misses')}>Misses</Button>
          <Button variant="success" onClick={() => setDataType('scores')}>Scores</Button>
        </div>
      </div>
    </div>
  );
