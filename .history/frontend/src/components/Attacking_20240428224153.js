...
function Attacking({ scoringZoneEfficiency, heatMapUrl, setDataType, selectedTeam }) {
  ...
  const sortedPlayerStats = React.useMemo(() => {
    ...
    if (selectedTeam) {
      // Prioritize players from the selected team
      sortableItems.sort((a, b) => {
        if (a.TeamName === selectedTeam && b.TeamName !== selectedTeam) return -1;
        if (a.TeamName !== selectedTeam && b.TeamName === selectedTeam) return 1;
        return 0;
      });
    }
  ...
  }, [playerStats, sortConfig, selectedTeam]); // Add selectedTeam as a dependency
