...
def get_pressure_map(team):
    path = "datasets/Pressure/{team}_Pressure_Map.png"
    return jsonify({'url': generate_signed_url(path)})

@app.route('/tacklesmaps/<team>')
def get_tackle_map(team):
    path = "datasets/Tackles/{team}_unsuccesful_tackles.png"
    return jsonify({'url': generate_signed_url(path)})

@app.route('/blockedmaps/<team>')
def get_blocked_map(team):
    path = "datasets/Blocked/{team}_block_points.png"
    return jsonify({'url': generate_signed_url(path)})

@app.route('/sdpmchart/<team>')
def get_sdpm_chart(team):
    path = "datasets/SDPM/{team}_SDPM_Chart.png"
    return jsonify({'url': generate_signed_url(path)})

@app.route('/asdmap/<team>')
def get_asd_map(team):
    path = "datasets/ASD/{team}_average_shot_positions.png"
    return jsonify({'url': generate_signed_url(path)})

@app.route('/ppmaps/<team>')
def get_pp_map(team):
    path = "datasets/PP/{team}_pressure_points.png"
    return jsonify({'url': generate_signed_url(path)})

@app.route('/heatmaps/<team>/<category>')
def get_heatmap(team, category):
    path = "datasets/{category.capitalize()}/{team}_{category.lower()}_heat_map.jpg"
    return jsonify({'url': generate_signed_url(path)})
